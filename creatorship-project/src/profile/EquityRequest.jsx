...
<div className="container">
  <h2 className="title">Equity Requests</h2>
  <select 
    className="filter"
    value={filter} 
  ...
  {filteredRequests.map(request => (
    <div key={request._id} className="requestCard">
      {userType === 'business' ? (
        <div>
          <p className="requestInfo"><span className="label">From Creator ID:</span> {request.creatorId}</p>
          <p className="requestInfo"><span className="label">Service Provider Name:</span> {request.businessName}</p>
          <p className="requestInfo"><span className="label">Creator Name:</span> {request.creatorName}</p>
          <p className="requestInfo"><span className="label">Creator Email:</span> {request.creatorEmail}</p>
          <p className="requestInfo"><span className="label">Equity:</span> {request.equity}%</p>
          <p className="requestInfo">
            <span className="label">Status:</span> 
            <span className={`status status${request.status.charAt(0).toUpperCase() + request.status.slice(1)}`}>
              {request.status}
            </span>
          </p>
        ...
        <div>
          <p className="requestInfo"><span className="label">From Business ID:</span> {request.businessId}</p>
          <p className="requestInfo"><span className="label">Business Name:</span> {request.businessName}</p>
          <p className="requestInfo"><span className="label">Creator Name:</span> {request.creatorName}</p>
          <p className="requestInfo"><span className="label">Equity:</span> {request.equity}%</p>
          <p className="requestInfo">
            <span className="label">Status:</span> 
